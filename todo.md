...
- [x] Noto Sans JP フォントファイルの取得（Google Fonts API 経由）
- [x] `src/utils/fonts.ts` の実装
- [x] `next/og` の `ImageResponse` にフォントを適用
- [x] フォント読み込みの動作確認
...
- [x] `?original=true` クエリパラメータの処理実装（元画像取得）
- [x] `?debugInfoOnly=true` クエリパラメータの処理実装（デバッグ情報取得）
- [x] 減色前後の比較画像生成機能（`/debug` ページで実装）
- [x] 処理時間の測定と表示機能
- 注: `?debug=true` は実装されていないが、`?original=true` と `/debug` ページで同等の機能を実現

### 3. 🧪 詳細テスト（優先度: 中）

本番環境に近い条件でのテストとパフォーマンス検証。

- [x] エラーハンドリングの詳細テスト（基本動作確認完了）
- [x] パフォーマンステスト（処理時間の測定は実装済み）
...
- [ ] `vercel.json` の設定確認（必要に応じて作成）
- [ ] 環境変数の確認
- [ ] Vercel へのデプロイ
- [ ] 本番環境での動作確認
- [ ] エラーログの監視設定

### 5. 📝 ドキュメント整備（優先度: 低）✅ ほぼ完了

プロジェクトの保守性向上のためのドキュメント作成。

- [x] README.md の更新（セットアップ手順、API 仕様）
- [x] コードコメントの追加（JSDoc - 主要な関数に実装済み）
- [ ] アーキテクチャ説明の追加（オプション）
